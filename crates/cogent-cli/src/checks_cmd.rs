...
pub fn dispatch_tool(name: &str, path: &str, recursive: bool, format: &str) -> i32 {
    ...
    match name {
        ...
        "riskmap" | "risk" => run_standalone_check(name, format, move || cogent_engine::checks::check_riskmap(&p, false, 50.0)),
...
mod tests {
    ...
    fn test_dir() -> String {
        ...
        std::path::Path::new(manifest).join("tests").to_string_lossy().to_string()
    }
    ...
    fn test_dispatch_typecov() {
        ...
        assert!(code == 0 || code == 1, "typecov should complete with 0 or 1, got {}", code);
    }

    #[test]
    fn test_dispatch_deadcode() {
        let code = dispatch_tool("deadcode", &test_dir(), true, "json");
        assert!(code == 0 || code == 1, "deadcode should complete with 0 or 1, got {}", code);
    }

    #[test]
    fn test_dispatch_dupfind() {
        let code = dispatch_tool("dupfind", &test_dir(), false, "json");
        assert!(code == 0 || code == 1, "dupfind should complete with 0 or 1, got {}", code);
    }

    #[test]
    fn test_dispatch_crypto() {
        let code = dispatch_tool("crypto", &test_dir(), true, "json");
        assert!(code == 0 || code == 1, "crypto should complete with 0 or 1, got {}", code);
    }

    #[test]
    fn test_dispatch_taint() {
        let code = dispatch_tool("taint", &test_dir(), false, "json");
        assert!(code == 0 || code == 1, "taint should complete with 0 or 1, got {}", code);
    }
    ...
    fn test_dispatch_vulnscan() {
        ...
        assert!(code == 0 || code == 1, "vulnscan should complete, got {}", code);
    }
    ...
    fn test_dispatch_outdated() {
        ...
        assert!(code == 0 || code == 1, "outdated should complete, got {}", code);
    }

    #[test]
    fn test_dispatch_propcov() {
        let code = dispatch_tool("propcov", &test_dir(), false, "json");
        assert!(code == 0 || code == 1, "propcov should complete, got {}", code);
    }
    ...
    fn test_dispatch_coupling() {
        ...
        assert!(code == 0 || code == 1, "coupling should complete, got {}", code);
    }
    ...
    fn test_dispatch_riskmap() {
        ...
        assert!(code == 0 || code == 1, "riskmap should complete, got {}", code);
    }
}
