...
impl ToolRegistry {
    ...
    pub fn run_check(
        ...
        match name {
            ...
            "secrets" => Some(check_secrets_with_excludes(path, recursive, thresholds.max_secrets, &thresholds.secrets_exclude_paths, &crate::DefaultToolRunner)),
            ...
            "halstead" => Some(check_halstead(path, recursive, thresholds.max_halstead_bugs)),
            "cohesion" => Some(check_cohesion(path, recursive, thresholds.max_cohesion)),
            "comments" => Some(check_comments(path, recursive, thresholds.min_comment_ratio)),
            ...
            "vulnscan" => Some(check_vulnscan(path, thresholds.max_vuln_critical, thresholds.max_vuln_high)),
            "licenses" => Some(check_licenses(path, thresholds.max_license_violations)),
            "access-control" => Some(check_access_control(path, recursive, thresholds.max_access_control, &thresholds.access_control_exclude_paths, &crate::DefaultToolRunner)),
            ...
            "complexity" => Some(check_complexity(path, recursive, thresholds.min_complexity, thresholds.max_complexity_violations)),
            "crap" => Some(check_crap(path, recursive, &thresholds.coverage_path, thresholds.max_crap)),
            "observability" => Some(check_observability(path, recursive, thresholds.max_observability)),
            "test-quality" => Some(check_test_quality(path, recursive, thresholds.max_test_quality)),
            "design-docs" => Some(check_design_docs(path)),
            "debuggability" => Some(check_debuggability(path, recursive, thresholds.max_debuggability)),
...
impl Default for ToolRegistry {
    fn default() -> Self {
        ...
        let entries = [
            ...
            AuditTool::new("outdated", "outdated", "outdated", "Dependency freshness — major version drift").with_recursive(false),
            AuditTool::new("observability", "observability", "cogent-cli", "Structured logging observability").with_recursive(true),
            AuditTool::new("test-quality", "test-quality", "cogent-cli", "Test quality & non-determinism detection").with_recursive(true),
            AuditTool::new("design-docs", "design-docs", "cogent-cli", "Design documentation pillar check").with_recursive(false),
            AuditTool::new("debuggability", "debuggability", "cogent-cli", "Contextless unwrap detection").with_recursive(true),
...
mod tests {
    ...
    fn test_run_check_arms_match_registry_entries() {
        ...
        let run_check_tools = [
            "taint", "secrets", "deadcode", "coupling", "linelen",
            "dupfind", "riskmap", "propcov", "fuzz", "halstead",
            "cohesion", "comments", "errhandle", "typecov", "sast",
            "crypto", "vulnscan", "licenses", "access-control",
            "supply-chain", "outdated", "debt", "doccov", "complexity",
            "crap", "observability", "test-quality", "design-docs",
            "debuggability",
        ];
